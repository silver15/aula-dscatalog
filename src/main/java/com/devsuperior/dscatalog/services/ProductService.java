...
public class ProductService {
	...
	public Page<ProductDTO> findAllPaged(String categoryId, String name, Pageable pageable) {
		...
		List<Product> products = repository.searchProductsWithCategories(page.map(x -> x.getId()).toList());

		return page.map(x -> {
			Product prod = products.stream().filter(p -> p.getId().equals(x.getId())).findFirst().get();
